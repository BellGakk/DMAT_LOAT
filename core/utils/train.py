...
class Trainer(object):
    ...
    def mart_loss(self, x, y, epoch, beta):
        """
        MART training.
        """
        loss, batch_metrics = mart_loss(self.model, x, y, epoch, self.reg_ep, self.more_reg, self.optimizer, step_size=self.params.attack_step, 
                                        epsilon=self.params.attack_eps, perturb_steps=self.params.attack_iter, 
                                        beta=beta, attack=self.params.attack)
        return loss, batch_metrics  
