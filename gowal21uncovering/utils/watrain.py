...
class WATrainer(Trainer):
    ...
    def __init__(self, info, args):
        ...
        seed(args.seed)
        self.reg_ep = self.params.num_adv_epochs
        self.more_reg = self.params.more_reg
            ...
            if self.params.consistency:
                ...
                if adversarial:
                    if self.params.beta is not None and self.params.mart:
                        loss, batch_metrics = self.mart_loss(x, y, epoch, beta=self.params.beta)
                    elif self.params.beta is not None and self.params.LSE:
                        loss, batch_metrics = self.trades_loss_LSE(x, y, epoch, beta=self.params.beta)
                    elif self.params.beta is not None:
                        loss, batch_metrics = self.trades_loss(x, y, epoch, beta=self.params.beta)
                    else:
    ...
    def trades_loss(self, x, y, epoch, beta):
        """
        TRADES training.
        """
        loss, batch_metrics = trades_loss(self.model, x, y, epoch, self.reg_ep, self.more_reg, self.optimizer, step_size=self.params.attack_step, 
    ...
    def trades_loss_LSE(self, x, y, epoch, beta):
        """
        TRADES training with LSE loss.
        """
        loss, batch_metrics = trades_loss_LSE(self.model, x, y, epoch, self.reg_ep, self.more_reg, self.optimizer, step_size=self.params.attack_step, 
                                          epsilon=self.params.attack_eps, perturb_steps=self.params.attack_iter, 
                                          beta=beta, attack=self.params.attack, label_smoothing=self.params.ls,
